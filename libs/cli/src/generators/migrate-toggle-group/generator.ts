...
function updateBrainImports(tree: Tree) {
	visitFiles(tree, '/', (path) => {
		...
		if (
			...
			content.includes("from 'grg-ui-ui/brain/toggle'") &&
			!content.includes("from 'grg-ui-ui/brain/toggle-group'")
		) {
			// Remove BrnToggleGroupModule from the toggle import
			content = content.replace(
				/import\s+\{\s*([^{}]*BrnToggleGroupModule[^{}]*)\s*\}\s+from\s+['"]grg-ui-ui\/brain\/toggle['"];/g,
				(_match, importList) => {
					...
					if (newImportList.length > 0) {
						return `import { ${newImportList} } from 'grg-ui-ui/brain/toggle';`;
					} else {
			...
			if (lastImportEndIndex > 0) {
				...
				const newImport = `import { BrnToggleGroupComponent, BrnToggleGroupItemDirective } from 'grg-ui-ui/brain/toggle-group';\n\n`;
...
function updateHlmImports(tree: Tree) {
	visitFiles(tree, '/', (path) => {
		...
		if (
			content.includes("from 'grg-ui-ui/ui-toggle-helm'") &&
			!content.includes("from 'grg-ui-ui/ui-toggle-group-helm'")
		) {
			// Replace HlmToggleDirective and HlmToggleGroupModule with the directives
			// Handle case with HlmToggleDirective
			content = content.replace(
				/import\s+\{\s*HlmToggleDirective\s*,\s*HlmToggleGroupModule\s*\}\s+from\s+['"]grg-ui-ui\/ui-toggle-helm['"];/g,
			...
			content = content.replace(
				/import\s+\{\s*HlmToggleGroupModule\s*\}\s+from\s+['"]grg-ui-ui\/ui-toggle-helm['"];/g,
				"import { HlmToggleGroupDirective, HlmToggleGroupItemDirective } from 'grg-ui-ui/ui-toggle-group-helm';",
			);
...
function replaceSelector(_tree: Tree) {
	visitFiles(_tree, '/', (path) => {
		...
		// if file contains grg-ui-ui/ui-toggle-group-helm
		if (content.includes('grg-ui-ui/ui-toggle-group-helm')) {
